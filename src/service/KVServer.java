...
public class KVServer {
    ...
    private void createPutContext() {
        server.createContext("/put", (h) -> {
            try {
                System.out.println("\n/put");
                if (!hasAuth(h)) {
                    System.out.println("Запрос неавторизован, нужен параметр в query API_KEY со значением апи-ключа");
                ...
                switch (h.getRequestMethod()) {
                    case "POST":
                        String key = h.getRequestURI().getPath().substring("/put/".length());
                        if (key.isEmpty()) {
                            System.out.println("Key для сохранения пустой. key указывается в пути: /put/{key}");
                            h.sendResponseHeaders(400, 0);
                            return;
                        }
                        ...
                        break;
                    default:
                        System.out.println("/put ждёт POST-запрос, а получил: " + h.getRequestMethod());
                        h.sendResponseHeaders(405, 0);
                }
            } finally {
                h.close();
